...
def compute_annotator_reliability(df):
    maj = df.groupby(['query_id', 'doc_id']).apply(simple_majority_vote).reset_index(name='majority')
...
def aggregate_judgements(df: pd.DataFrame, method='advanced', strategy='confidence_weighted', annotator_reliability=None) -> pd.DataFrame:
    ...
    for (qid, did), group in grouped:
        if method == 'majority':
            ...
            score = advanced_aggregation(group, strategy=strategy, annotator_reliability=annotator_reliability)
            agreement = group['confidence'].mean()
        ...
        aggregated.append({
            ...
            'std_score': round(float(agreement), 3)
        })
